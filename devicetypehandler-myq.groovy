...
metadata {
	definition (
		...
		capability "Door Control"			//	Allow control of a switch
	...
	tiles {
		standardTile("toggle", "device.door", width: 2, height: 2) {
			...
			state("closing", label:'${name}', icon:"st.doors.garage.garclosingage-open", backgroundColor:"#00A0DC")
		}
		...
		standardTile("closed", "device.door", inactiveLabel: false, decoration: "flat") {
...
//	-------------
def open() {
	if(get_DEBUG() == "on") { log.trace "open()..." }
	
	//	Updating the event/button within ST...
	if(get_DEBUG() == "on") { log.debug "[open()] - executing sendEvent opening" }
	sendEvent(name: "door", value: "opening")//, isStateChange: true, display: true, displayed: true)
	//	Executing the API call...
	if(get_DEBUG() == "on") { log.debug "[open()] - executing callURL_DeviceAction to open the door" }
	callURL_DeviceAction('open')
	//	Marking it as FINISHED opening...
    runIn(20, finishOpening)
}
def finishOpening() {
	if(get_DEBUG() == "on") { log.trace "finishOpening()..." }
	
	if(get_DEBUG() == "on") { log.debug "[finishOpening()] - executing sendEvent open" }
	sendEvent(name: "door", value: "open")//, isStateChange: true, display: true, displayed: true)
...
def close() {
	if(get_DEBUG() == "on") { log.trace "close()..." }
	
	//	Updating the event/button within ST...
	if(get_DEBUG() == "on") { log.debug "[close()] - executing sendEvent closing" }
    sendEvent(name: "door", value: "closing")//, isStateChange: true, display: true, displayed: true)
	//	Executing the API call...
	if(get_DEBUG() == "on") { log.debug "[close()] - executing callURL_DeviceAction to close the door" }
	callURL_DeviceAction('close')
	//	Marking it as FINISHED closing...
	runIn(40, finishClosing)
}
def finishClosing() {
	if(get_DEBUG() == "on") { log.trace "finishClosing()..." }
	
	if(get_DEBUG() == "on") { log.debug "[finishClosing()] - executing sendEvent closed" }
    sendEvent(name: "door", value: "closed")//, isStateChange: true, display: true, displayed: true)
